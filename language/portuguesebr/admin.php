<?php
/**
* Portuguesebr language constants used in admin section of the module
*
* @copyright	The SmartFactory http://www.smartfactory.ca
* @license		http://www.gnu.org/licenses/old-licenses/gpl-2.0.html GNU General Public License (GPL)
* @since		1.0
* @package		Imtranslating
* @author		marcan <marcan@smartfactory.ca>
* @version		$Id: admin.php 1394 2008-05-22 16:21:43Z marcan $
* @translation        GibaPhp
*/

if (!defined("XOOPS_ROOT_PATH")) {
 	die("Caminho Raiz do ImpressCMS não está definido");
}


define("_AM_IMTRANSL_FROM_LANG", "Idioma de Referência");
define("_AM_IMTRANSL_TO_LANG", "Língua da tradução");
define("_AM_IMTRANSL_MODULE", "Módulo");
define("_AM_IMTRANSL_COREFILES", "Arquivos Core/Núcleo");
define("_AM_IMTRANSL_INSTALLFILES", "Arquivos de Instalação");
define("_AM_IMTRANSL_GO", "Continua");
define("_AM_IMTRANSL_CANCEL", "Cancelar");
define("_AM_IMTRANSL_ORIGINAL", "%s definição para %s");
define("_AM_IMTRANSL_JOB", "Constante Ausente em %s");
define("_AM_IMTRANSL_TRANSLATION", "Tradução");
define("_AM_IMTRANSL_COMMENT", "//Nova constante criada através do IMtranslating");
define("_AM_IMTRANSL_ZIP", "Zip/Compactar e download");
define("_AM_IMTRANSL_DONE", "Você Terminou!");
// Log messages
define("_AM_IMTRANSL_READ_ERR", "Não é possível encontrar os arquivos de referência.");
define("_AM_IMTRANSL_PARSE_ERR", "Pasta de Referência não contém os arquivos de idioma.");
define("_AM_IMTRANSL_NO_TRANS_NEEDED_ERR", "Todas as constantes de idioma já estão definidas.");
define("_AM_IMTRANSL_WRITE_ERR", "Não é possível escrever no arquivo %s.");

?>